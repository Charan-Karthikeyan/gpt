...
%token 
 ...
 STARTTOK 21 INCLUDETOK 22 EXCLUDETOK 23 GOALTOK 24
 EOFTOK 258
...
goal_param_tail :  goal_list
                |  ident_list
                /* we can list the goal states by name or number */
;
...
ident_list	: ident_list STRINGTOK
		   ...
		   if (curMnemonic != nt_goal)
		     ...
		     gGoalList[gNumGoals++] = H_lookup( $2->theValue.theString, nt_state );
		...
		| STRINGTOK
		   ...
		   if (curMnemonic != nt_goal)
		     ...
		     gGoalList[gNumGoals++] = H_lookup( $1->theValue.theString, nt_state );
                }
;
goal_list       : goal_list INTTOK
                {
		   gGoalList[gNumGoals++] = $2->theValue.theInt;
		}
                | INTTOK
                {
		   gGoalList[gNumGoals++] = $1->theValue.theInt;
		}
;
prob		: INTTOK
		{
