...
public:
  ...
  virtual ~resultClass() = 0;

  // methods 
  void startTimer();
  void stopTimer();
  unsigned long getElapsedSeconds();
  unsigned long getElapsedUSeconds();
...
protected:
  ...
  struct stepClass {
    ...
    stepClass( int s, int a, int o ) { state = s; action = a; observation = o; }
  };
...
public:
  ...
  virtual void pop_back() { stepList.pop_back(); --numMoves; }
  ...
  virtual void pop_front() { stepList.pop_front(); --numMoves; }
};
